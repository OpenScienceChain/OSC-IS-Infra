  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = aws_codebuild_project.lifecycle.arn
      },
      {
        Effect   = "Allow"
...
resource "aws_sfn_state_machine" "stop" {
  ...
  definition = jsonencode({
    ...
    States = {
      ...
      Sweep = {
        Type       = "Task", Resource = "arn:aws:states:::codebuild:startBuild.sync"
        Parameters = { ProjectName = aws_codebuild_project.lifecycle.name, EnvironmentVariablesOverride = [{ Name = "ACTION", Value = "SWEEP", Type = "PLAINTEXT" }, { Name = "RUN_ID", "Value.$" = "$.runId", Type = "PLAINTEXT" }] }
        Retry      = [{ ErrorEquals = ["States.TaskFailed"], IntervalSeconds = 60, BackoffRate = 2, MaxAttempts = 3 }]
        Next       = "MarkClosed"
      }
      MarkClosed = {
