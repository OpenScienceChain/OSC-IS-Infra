...
class RuntimeTopologyTests(unittest.TestCase):
    def test_three_az_nodes_and_multi_az_rabbitmq(self) -> None:
        ...
        self.assertIn('tags = { Name = "${local.name_prefix}-cloudfront-origin" }', network)
        self.assertIn('resource "aws_security_group" "alb_backend"', network)
        self.assertIn('"elbv2.k8s.aws/cluster"', network)
        self.assertIn('"elbv2.k8s.aws/resource" = "backend-sg"', network)
        self.assertIn('resource "aws_ec2_tag" "eks_cluster_security_group"', eks)
        self.assertIn("for_each = local.required_tags", eks)
...
class LifecycleContractTests(unittest.TestCase):
    def test_exact_account_region_schedule_and_time_bounded_cost_controls(self) -> None:
        ...
        deployment = read("platform/aws/deploy-aws-platform.ps1")
        self.assertNotIn("$statePath", deployment)
        self.assertNotIn('"-state=', deployment)
        self.assertIn("terraform -chdir=terraform/usrse26-eks output -raw rabbitmq_amqps_endpoint", deployment)
        self.assertIn("terraform -chdir=terraform/usrse26-eks output -raw vpc_id", deployment)
        validation = read("platform/aws/validate-aws-platform.ps1")
        self.assertIn('"OSC_RUNTIME_SECRET_DIR=/tmp/osc-is-$RunId-runtime-secrets"', validation)
        gitops_validation = read("platform/scripts/validate-local-gitops.sh")
        self.assertIn('DRIFT_REPLICAS=$((BASELINE_REPLICAS + 1))', gitops_validation)
        self.assertIn('--replicas="${DRIFT_REPLICAS}"', gitops_validation)
        self.assertIn('!= "${BASELINE_REPLICAS}"', gitops_validation)
        self.assertIn(
    ...
    def test_static_fallback_and_retention_survive_runtime_contract(self) -> None:
        edge = read("terraform/usrse26-control/edge.tf")
        lifecycle = read("terraform/usrse26-control/lifecycle.tf")
        lifecycle_runner = read("platform/lifecycle/osc_demo_lifecycle.py")
        ...
        self.assertIn('"CustomHeaders": {"Quantity": 0}', lifecycle_runner)
        self.assertIn('"OriginShield": {"Enabled": False}', lifecycle_runner)
        self.assertIn('"FieldLevelEncryptionId": ""', lifecycle_runner)
        self.assertIn('"GrpcConfig": {"Enabled": False}', lifecycle_runner)
        self.assertIn('"cloudfront", "list-vpc-origins"', lifecycle_runner)
        self.assertIn("Multiple CloudFront VPC origins match", lifecycle_runner)
        self.assertIn('config["CustomErrorResponses"] = {"Quantity": 0}', lifecycle_runner)
        self.assertIn('"ErrorCode": 403, "ResponsePagePath": "/index.html"', lifecycle_runner)
        self.assertIn('"ErrorCode": 404, "ResponsePagePath": "/index.html"', lifecycle_runner)
        self.assertIn('os.utime(extracted, None)', lifecycle_runner)
        self.assertIn('"DEMO_MODE": True', lifecycle_runner)
        self.assertIn('"API_BASE_URL": "/api/v1"', lifecycle_runner)
        ...
        self.assertIn('"ExpiresAt": runtime_expires_at', lifecycle_runner)
        self.assertIn('runtime_variables.read_text(encoding="utf-8")', lifecycle_runner)
        self.assertIn("tag_index_entry_is_active", lifecycle_runner)
        self.assertIn('"Name=tag:Environment,Values=ephemeral"', lifecycle_runner)
        self.assertEqual(lifecycle_runner.count("str(uuid.uuid4())"), 2)
        self.assertNotIn("uuid.uuid5", lifecycle_runner)
